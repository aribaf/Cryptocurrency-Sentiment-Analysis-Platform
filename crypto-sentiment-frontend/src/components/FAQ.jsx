        ...
        <div className="space-y-4">
          {list.map((it, i) => (
            <div key={i} className="rounded-lg overflow-hidden border border-gray-800 shadow-xl">
              ...
              <button 
                className="w-full px-6 py-5 text-left flex items-center justify-between bg-black text-white hover:bg-[#1a1a1a] transition-colors"
                onClick={() => setOpenIndex(openIndex === i ? null : i)}
              >
                <span className="font-semibold text-lg">{it.q}</span>
                <span className="text-[#ff522f] text-2xl font-light ml-4">
                  {openIndex === i ? "—" : "+"}
                </span>
              </button>
              ...
              <div 
                className={`px-6 transition-all duration-300 ease-in-out bg-[#101010] ${
                  openIndex === i 
